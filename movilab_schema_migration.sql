...
-- ============================================================
-- Movi‑Lab: Consolidated, Idempotent Schema & Migration Patch
-- Generated: 2025-09-11
-- Notes:
--  - Safe to re-run (uses IF NOT EXISTS, DO $$...$$ guards)
--  - Unifies appointments model (client+coach+time window)
--  - Removes service_items duplication (keeps public.service_items)
--  - Adds guards, indexes, triggers, and robust seeds
-- ============================================================
...
-- Coaches (missing in original script)
...
-- ---------- 2) Service Items (single definition, in public schema) ----------
...
CREATE TABLE IF NOT EXISTS coach_availability_rules (
  ...
  start_minute INT NOT NULL CHECK (start_minute BETWEEN 0 AND 1439),       -- minutes since midnight
...
CREATE TABLE IF NOT EXISTS coach_availability_exceptions (
  ...
  note         TEXT,
  is_available BOOLEAN NOT NULL DEFAULT false,  -- false = fully blocked; true = window defined
  created_at   TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Customer requests a slot for a given service
...
-- Coaches notified for a request
...
-- Align existing table if it pre-existed (add missing columns/constraints)
ALTER TABLE appointments
  ADD COLUMN IF NOT EXISTS client_id      INT REFERENCES clients(id)   ON DELETE CASCADE,
  ADD COLUMN IF NOT EXISTS service_id     INT REFERENCES services(id)  ON DELETE RESTRICT,
  ADD COLUMN IF NOT EXISTS coach_id       INT REFERENCES coaches(id)   ON DELETE RESTRICT,
...
-- Anti double-booking (coach)
ALTER TABLE appointments
  ADD COLUMN IF NOT EXISTS slot tsrange GENERATED ALWAYS AS (tsrange(starts_at, ends_at, '[)')) STORED;
...
-- Default admin (DEMO password hash placeholder)
INSERT INTO admins (email, password_hash, name)
VALUES ('admin@movi-lab.fr', '$2b$10$rOzJqQqQqQqQqQqQqQqQqO', 'Administrateur')
ON CONFLICT (email) DO NOTHING;

-- Service items for M-Starter
